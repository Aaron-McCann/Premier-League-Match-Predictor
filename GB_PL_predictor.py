...
# train the model
xgb_model.fit(train_rolling[all_predictors], train_rolling["target"])

# cross-validate the model and print accuracy and precision
accuracy_scores_xgb = cross_val_score(xgb_model, train_rolling[all_predictors], train_rolling["target"], cv=5, scoring='accuracy')
precision_scores_xgb = cross_val_score(xgb_model, train_rolling[all_predictors], train_rolling["target"], cv=5, scoring='precision')

# log the cross-validation results
print(f"XGBoost Cross-Validation Accuracy: {accuracy_scores_xgb.mean()} (±{accuracy_scores_xgb.std()})")
print(f"XGBoost Cross-Validation Precision: {precision_scores_xgb.mean()} (±{precision_scores_xgb.std()})")

# predict test set results and evaluate accuracy/precision
preds_xgb = xgb_model.predict(test_rolling[all_predictors])
accuracy_xgb = accuracy_score(test_rolling["target"], preds_xgb)
precision_xgb = precision_score(test_rolling["target"], preds_xgb)

# log the test set performance
print(f"Test Set - Accuracy: {accuracy_xgb}")
print(f"Test Set - Precision: {precision_xgb}")
...
# Define non-rolling predictors (features that are not related to rolling averages)
...
future_predictions = xgb_model.predict(future_matches[all_future_predictors])
...
print(f"Predictions saved to new_future_predictions.csv")
